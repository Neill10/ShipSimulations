...
public class Simulation {
    private static String[] listOfAddress = {
            /*
    ...
    //Ask Mr.Das about how to generate addresses (whether it be a preset list or can be a non-real location)
    /*
    private static String generateAddress(String streetName, String City, )
    {
        String address = "";
        String streetNum = "";
        int numLength = (int)((Math.random() * 6) + 1);//1-6 in
        streetNumber(numLength);
        address = address + streetNum + " ";



    }

    private static String streetNumber(int length)
    {
        String streetNum = "";
        for(int i = 0; i < length;i++)
        {
            if(i == 0) {
                int random = (int) ((Math.random() * 9) + 1);//1-9 in
                streetNum = streetNum + random;
            }
            else
            {
                int random = (int) ((Math.random() * 9)); //0-9 in
                streetNum = streetNum + random;
            }
        }
        return streetNum;
